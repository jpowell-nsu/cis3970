...
public class Driver {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		PrimeCounter pc = new PrimeCounter(1, 50_000_000);
		Thread vt = Thread.ofVirtual().start(pc);
		vt.join();
		System.out.println(vt + " " +  pc.getTime());
...
//			vtpool[i] = Thread.ofVirtual().start(new PrimeCounter(1, 50000000));
		}
	
		for (int i = 0; i < vtpool.length; i++) {
			vtpool[i].join();
			System.out.println(vtpool[i] + " " +  pool[i].getTime());
		}
	 */
	}
